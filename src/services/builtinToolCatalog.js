...
export const BUILTIN_TOOLS = [
  ...
  {
    id: 'opencloud_search_artifacts',
    name: 'Search DI Artifacts in OpenCloud',
    description: 'Full-text search across OpenCloud drives for DI artifacts. Uses Bleve indexing + Tika content extraction. Supports KQL queries (e.g. Tags:di_forecast, name:report).',
    category: TOOL_CATEGORY.UTILITY,
    keywords_en: ['search', 'find', 'artifact', 'opencloud', 'query', 'full-text', 'locate'],
    keywords_zh: ['搜尋', '查找', '搜索', '全文', '找文件'],
    module: './opencloudClientService',
    method: 'searchAllDrives',
    tier: 'tier_c',
    required_datasets: [],
    output_artifacts: [],
    depends_on: [],
    needs_dataset_profile: false,
    input_schema: {
      query: 'string (search query, supports KQL: Tags:di_forecast, name:report)',
      opts: 'object|null (optional: { driveId, limit })',
    },
  },

  {
    id: 'opencloud_distribute_report',
    name: 'Distribute Report via OpenCloud',
    description: 'Upload a report to OpenCloud and send share invitations to team members for automated report distribution.',
    category: TOOL_CATEGORY.UTILITY,
    keywords_en: ['distribute', 'share', 'report', 'send', 'team', 'publish', 'opencloud'],
    keywords_zh: ['分發', '發送', '分享', '報告', '團隊', '發佈'],
    module: './opencloudArtifactSync',
    method: 'distributeReport',
    tier: 'tier_c',
    required_datasets: [],
    output_artifacts: ['opencloud_file_ref'],
    depends_on: [],
    needs_dataset_profile: false,
    input_schema: {
      reportResult: 'object (from generateReport())',
      driveId: 'string',
      taskId: 'string',
      opts: 'object|null (optional: { recipients, employeeName })',
    },
  },

  {
    id: 'opencloud_find_by_tag',
    name: 'Find Artifacts by Tag in OpenCloud',
    description: 'Search for DI artifacts in OpenCloud by artifact type tag (di:forecast, di:plan, di:risk, etc.).',
    category: TOOL_CATEGORY.UTILITY,
    keywords_en: ['tag', 'find', 'artifact', 'type', 'opencloud', 'classify'],
    keywords_zh: ['標籤', '分類', '查找', '類型'],
    module: './opencloudArtifactSync',
    method: 'findArtifactsByType',
    tier: 'tier_c',
    required_datasets: [],
    output_artifacts: [],
    depends_on: [],
    ...
    input_schema: {
      driveId: 'string',
      artifactType: 'string (e.g. forecast_series, plan_table, risk_adjustments)',
    },
  },

  {
    id: 'opencloud_watch_folder',
    name: 'Watch OpenCloud Folder',
    description: 'Start watching an OpenCloud folder for new or modified files using SSE (real-time) or polling. Emits events when changes are detected.',
    category: TOOL_CATEGORY.UTILITY,
    keywords_en: ['watch', 'monitor', 'folder', 'opencloud', 'file', 'detect', 'trigger'],
    keywords_zh: ['監控', '監視', '文件夾', '偵測', '觸發'],
    module: './opencloudFileWatcher',
    method: 'startWatchingAuto',
    tier: 'tier_c',
    required_datasets: [],
    output_artifacts: [],
    depends_on: [],
    ...
    input_schema: {
      driveId: 'string',
      folderId: 'string|null',
      opts: 'object|null (optional: { filter, intervalMs, preferSSE })',
    },
  },
