...
function parseDateValue(val) {
  ...
  if (typeof val === 'number' && Number.isFinite(val) && Number.isInteger(val) && val >= 1000 && val <= 9999) {
    const d = new Date(Date.UTC(val, 0, 1));
    return Number.isNaN(d.getTime()) ? null : d;
  }
  if (typeof val === 'string') {
    const trimmed = val.trim();
    if (/^\d{4}$/.test(trimmed)) {
      const d = new Date(Date.UTC(Number(trimmed), 0, 1));
      return Number.isNaN(d.getTime()) ? null : d;
    }
  }
...
/**
 * Detect partial-year coverage: if the query results span multiple calendar years
 * but some years have far fewer months of data than others, CAGR and YoY comparisons
 * will be misleading because they assume full-year coverage.
 *
 * Example: Olist data starts Sep 2016 → 2016 has only 3 months, but 2017 has 12.
 * A CAGR from 2016→2018 reflects coverage expansion, not true business growth.
 */
export function checkPartialYearCoverage(rows, columns, sql) {
  const warnings = [];
  const dateCol = findDateColumn(columns);
  if (!dateCol) return warnings;

  // Build year → set of distinct months (use UTC to avoid timezone shifts)
  const yearMonths = {};
  for (const row of rows) {
    const d = parseDateValue(row[dateCol]);
    if (!d) continue;
    ...
    yearMonths[year].add(d.getUTCMonth());
  }

  const years = Object.keys(yearMonths).map(Number).sort((a, b) => a - b);
  if (years.length < 2) return warnings;

  const yearCoverage = years.map((y) => ({ year: y, months: yearMonths[y].size }));
  const maxMonths = Math.max(...yearCoverage.map((yc) => yc.months));

  // If all years show exactly 1 month, the date column likely holds year-only values
  // (e.g., EXTRACT(YEAR ...) result). We can't detect partial coverage from the rows alone.
  // In this case, check if SQL uses EXTRACT(YEAR) and warn the agent to verify coverage.
  if (maxMonths === 1 && years.length >= 2) {
    const sqlStr = typeof sql === 'string' ? sql : '';
    const isYearGrouped = /EXTRACT\s*\(\s*YEAR/i.test(sqlStr)
      || /GROUP\s+BY.*\byear\b/i.test(sqlStr);
    if (isYearGrouped) {
      warnings.push({
        id: 'partial_year_coverage',
        severity: 'high',
        category: 'completeness',
        column: dateCol,
        message: `Query groups by year (${years.join(', ')}) but individual years may have partial data coverage (not all 12 months). CAGR and YoY comparisons assume full-year data.`,
        instruction: `Before computing CAGR or YoY growth, run a separate query to check monthly coverage per year (e.g., SELECT year, COUNT(DISTINCT DATE_TRUNC('month', ...)) ...). Compute overall year coverage from the full dataset — NEVER infer it via MAX(covered_months) from region/entity aggregates. If any year has fewer than 3 months of data, do NOT use it as a CAGR endpoint at all — even per-month normalization is unreliable with 1-2 months. If the base year has < 3 months, use the next full year as the base instead. Prefer comparing only full-coverage years (e.g., 2017 full year vs 2018 Jan-Aug same-period).`,
      });
      return warnings;
    }
  }

  // Flag years with significantly fewer months than the best-covered year
  const partialYears = yearCoverage.filter((yc) => yc.months < maxMonths * 0.75);

  if (partialYears.length > 0) {
    const details = partialYears
      .map((yc) => `${yc.year} (${yc.months} month${yc.months !== 1 ? 's' : ''})`)
      .join(', ');
    const fullDetails = yearCoverage
      .map((yc) => `${yc.year}: ${yc.months} month${yc.months !== 1 ? 's' : ''}`)
      .join(', ');
    ...
    warnings.push({
      ...
      category: 'completeness',
      column: dateCol,
      message: `Partial-year data detected: ${details}. Full coverage: ${fullDetails}. Year-over-year comparisons and CAGR will be inflated by coverage differences, not true growth.`,
      instruction: `Do NOT compute CAGR or YoY growth rates using partial years as base or end periods. Compute overall year coverage from the full dataset — NEVER infer it via MAX(covered_months) from region/entity aggregates. Years with fewer than 3 months of data must NOT be used as CAGR endpoints even with per-month normalization — 1-2 months is too unreliable. Use only full-coverage years (${yearCoverage.filter((yc) => yc.months >= maxMonths * 0.75).map((yc) => yc.year).join(', ')}), or compare same-period months across years (e.g., Jan-Aug 2017 vs Jan-Aug 2018).`,
    });
  }
...
export const WARNING_ACKNOWLEDGMENT_PATTERNS = {
  ...
  small_sample: [/small sample|few.*observation|limited data|limited annual coverage|yearly observations?|annual points?|annual coverage|insufficient periods?.*time[-\s]?series inference|樣本.*小|觀察.*少|資料不足|年度觀測|年度資料點|年資料點|年度覆蓋有限|期間不足/i],
