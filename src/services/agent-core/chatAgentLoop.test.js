...
describe('chatAgentLoop analysis quality contract', () => {
  it('injects the English unit, derivation, coverage, and narrative rules', () => {
    const contract = buildAnalysisQualityContract({
      required_dimensions: ['地區營收', '年度變化', '前三地區'],
    });

    expect(contract).toContain('CANONICAL UNIT POLICY (MANDATORY):');
    expect(contract).toContain('Never convert a raw ratio-like value to percent unless the evidence explicitly provides a percent-form field or your SQL alias ends with "_pct".');
    expect(contract).toContain('EVIDENCE-SAFE DERIVATIONS:');
    expect(contract).toContain('REQUIRED COVERAGE FOR RANKING + TREND REQUESTS:');
    expect(contract).toContain('The answer contract includes annual change.');
    expect(contract).toContain('Never derive overall year coverage via `MAX(covered_months)` from region-level aggregates.');
    expect(contract).toContain('NARRATIVE SHAPE:');
    expect(contract).toContain('Never simulate tables in summary.');
    expect(contract).toContain('Use only ONE primary growth basis in headline and summary');
    expect(contract).toContain('If the requested CAGR or YoY metric is not reliable because of partial-year coverage');
  });
});
