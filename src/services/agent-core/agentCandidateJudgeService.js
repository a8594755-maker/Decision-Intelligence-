...
import { detectDomain, buildJudgeDomainCriteria } from '../data-prep/analysisDomainEnrichment.js';
import { computeQueryComplexity } from './agentExecutionStrategyService.js';
import { logEvent } from '../governance/auditService.js';
...
function buildFallbackDecision(primaryCandidate, secondaryCandidate) {
  ...
  return {
    ...
    confidence: bothBelowThreshold ? 0.35 : 0.6,
    reviewer: {
      provider: 'deterministic_fallback',
...
export async function judgeAgentCandidates({
  ...
  try {
    ...
    const decision = applyWinnerGuardrails({
      ...
      confidence: Math.max(Number(result?.parsed?.confidence || 0), 0.25),
      reviewer: {
