...
import { invokeAiProxy, invokeAiProxyStream } from '../ai-infra/aiProxyService.js';
import { trackLlmUsage } from '../../utils/llmUsageTracker.js';
import { detectToolGap, detectProactiveGap } from '../ai-infra/gapDetectionService.js';
import { generateToolBlueprint } from '../ai-infra/toolBlueprintGenerator.js';
import { getRecipeIndexForPrompt, findRecipeByUserMessage } from '../charts/chartRecipeCatalog.js';
import { getModelConfig, resolveProviderFromModel } from '../ai-infra/modelConfigService.js';
...
import {
  ...
  classifyToolError,
  failureDedupeKey,
} from '../memory/aiEmployeeMemoryService.js';
import { classifyQueryIntent } from '../ai-infra/queryIntentClassifier.js';
import { buildQueryPlan, formatQueryPlanForPrompt } from '../data-prep/queryPlannerService.js';
...
export function buildAnalysisQualityContract(answerContract = null) {
  const requiredDimensions = Array.isArray(answerContract?.required_dimensions)
    ...
    : [];
  const hasAnnualChangeRequirement = requiredDimensions.some((item) =>
    /annual|year|year-to-year|year over year|yearly|time change|年度|年度變化/.test(item)
  );

  return [
    'CANONICAL UNIT POLICY (MANDATORY):',
    'Use one display unit per metric across the entire brief.',
    'Never convert a raw ratio-like value to percent unless the evidence explicitly provides a percent-form field or your SQL alias ends with "_pct".',
    'If the evidence provides "overall_cagr" = 11.18, do not write 1118.0% unless you also computed and cited "overall_cagr_pct".',
    '',
    'EVIDENCE-SAFE DERIVATIONS:',
    'Do not compute new multipliers, shares, or point differences in prose unless that derived value already exists in the evidence.',
    'If a comparison matters but the exact derived number is not in evidence, describe the direction without inventing a derived figure.',
    '',
    'REQUIRED COVERAGE FOR RANKING + TREND REQUESTS:',
    'Cover all four: top entities, change over time, comparison vs overall, and limitations.',
    hasAnnualChangeRequirement
      ? 'The answer contract includes annual change. Include at least one explicit start-to-end or year-to-year sentence in summary or key_findings.'
      : 'If the request includes annual change, include at least one explicit start-to-end or year-to-year sentence in summary or key_findings.',
    'When checking year coverage, compute overall coverage independently from the full dataset, e.g. `COUNT(DISTINCT DATE_TRUNC(\'month\', ...))` by year on all rows.',
    'Never derive overall year coverage via `MAX(covered_months)` from region-level aggregates. That is not true overall coverage and will distort normalized growth comparisons.',
    '',
    'NARRATIVE SHAPE:',
    'Summary must be prose or bullet points only. Never simulate tables in summary.',
    'Use at most 3 exact numbers in summary; move raw values to metric_pills or tables.',
    'Use only ONE primary growth basis in headline and summary (for example: CAGR, comparable YoY growth, or normalized monthly growth).',
    'If a second growth lens is useful for methodology, move it to methodology_note or one secondary key_finding instead of mixing it into the main summary.',
    'If the requested CAGR or YoY metric is not reliable because of partial-year coverage, say so explicitly in plain English and name the substitute metric you are using instead.',
  ].join('\n');
}
...
export async function runAgentLoop({
  ...
  const importantInstructions = isAnalysisMode
    ? [
        ...
        '- After generate_chart succeeds: the card already shows title, metrics, highlights, and chart. Do NOT repeat them in full. Only add a short (2-4 sentence) business insight or actionable recommendation that is NOT already in the card. **Exception**: for histogram+quantile requests, always state the core cut-point values (P25, P50, P75, P90) explicitly in your final answer, even if the chart card already shows them. If there is nothing else to add, say "The chart is ready." or similar.',
        ...
        '',
        buildAnalysisQualityContract(answerContract),
        '',
        'Data Enrichment Rules:',
        '- When reporting metrics, include both absolute and relative context only when both forms already exist in the evidence or are explicitly computed in SQL/Python output.',
        ...
        '- For SQL queries returning aggregated metrics, fetch relative context (% of total, rank within category, vs overall average) in the same or follow-up query when the request requires comparison or prioritization.',
        '',
        'Data Provenance Rules:',
        '- CRITICAL: Only report numbers that come directly from query results or chart artifacts. If a number is an assumption or industry benchmark (e.g., DSO, marketing budget, warehouse cost, inventory holding cost %), you MUST explicitly label it as "assumption" — never present assumptions as data-backed findings.',
        '- Do NOT fabricate financial projections (capital requirements, marketing costs, system costs, headcount) unless the dataset contains those fields. If the dataset lacks the data, state "requires business input" instead of inventing numbers.',
        ...
        '- NUMBER FIDELITY: COPY exact numeric values from tool results into tables. In narrative, prefer business-formatted values unless the exact raw value is required for the requested comparison. Do not change unit conventions mid-brief.',
        ...
        '- Treat SQL expressions like POWER(end/start, 1/periods) - 1 as raw ratio-like outputs unless the evidence explicitly provides a percent-form field.',
        '- If the evidence column is named `*_cagr`, `*_growth`, or `*_ratio` and returns 11.18, keep 11.18 as the displayed value (or 11.18x only if that exact convention already exists in evidence). Do NOT rewrite it as 1118.0% unless you also computed and cited a `*_pct` field.',
        '- If you explicitly compute a percentage companion field such as `overall_cagr_pct`, then you may display that percentage form everywhere for that metric. Never mix raw ratio, x-multiple, and percentage forms within the same brief.',
        '- DETECTION RULE: If a column named *cagr*, *growth*, or *rate* has value > 1.0, treat it as a raw formula value or multiplier unless a dedicated percent field exists. Do NOT silently apply ×100.',
        '- If value < 1.0 and the evidence or column metadata indicates a decimal rate, you may display it as a percentage consistently.',
        ...
        '  ✗ BAD: "營收 CAGR 為 11.18，訂單中位數為 R$120。" (just restating pills)',
        '  ✓ GOOD: "營收在三年間呈爆發式增長（主因為 2017 年平台快速擴張），但極高的 CAGR 主要受低基期影響，2018 年增速已明顯放緩至 45%。" (interpretation + context)',
        '- FORMATTING: Format numbers for business readability. Use K/M/B suffixes for large numbers (e.g., "R$1.01M" not "1010271.37"). Round to at most 2 decimal places. Order counts should be integers (e.g., "7,451" not "7451.00"). Percentages should show 1 decimal (e.g., "+23.5%"). Do not switch a metric between ratio, x-multiple, and percent within the same brief.',
  ...
  for (let i = 0; i < maxIterations; i++) {
    ...
    for (const tc of response.tool_calls) {
      ...
      if (toolName === 'query_sap_data' && toolResult.success && toolResult.result?.rows) {
        try {
          import('../data-prep/dataInsightService.js').then(({ extractInsightsFromQueryResult }) => {
        ...
        try {
          ...
          if (metaEntries.length > 0) {
            const metaLines = metaEntries.map(([col, meta]) => {
              const parts = [`${col}: type=${meta.unit}`];
              if (meta.scope) parts.push(`scope=${meta.scope}`);
              if (meta.precision_note) parts.push('⚠️ has floating-point residuals');
              if (meta.unit === 'ratio_multiplier') {
                parts.push('⚠️ keep raw ratio-like unit unless a *_pct field exists');
              }
              if (meta.display_hint) parts.push(meta.display_hint);
              return `  - ${parts.join(', ')}`;
            });
            deferredGuidance.push({
              role: 'user',
              content: `📊 COLUMN METADATA for previous query:\n${metaLines.join('\n')}\nUse these unit types when interpreting values. Keep ratio_multiplier columns in their raw unit unless the evidence also includes an explicit percentage companion field such as *_pct.`,
            });
          }
        } catch { /* non-critical */ }
      }
      ...
      const resultContent = toolResult.success
        ? JSON.stringify(cleanToolResultFloats(summarizeToolResult(toolResult.result)), null, 2)
