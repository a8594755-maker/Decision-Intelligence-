...
export async function executeBomExplosionLegacy(userId, batchId, demandFgRows, bomEdgesRows, options = {}) {
  const { componentDemandService, componentDemandTraceService, forecastRunsService } = await import('../infra/supabaseClient');
  const { importBatchesService } = await import('../data-prep/importHistoryService');
