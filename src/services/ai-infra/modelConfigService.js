...
const INSIGHTS_DEFAULT = { provider: 'deepseek', model: 'deepseek-reasoner' };

/**
 * Get the Insights Hub agent model config.
 * No auto/fallback — user sets provider+model directly in Settings.
 * @returns {{ provider: string, model: string }}
 */
...
/**
 * Resolve the effective provider+model for Insights Hub agent.
 * @returns {{ provider: string, model: string }}
 */
export function getResolvedInsightsHubModel() {
  return getInsightsHubModelConfig();
}

/**
 * Set Insights Hub agent model.
 */
