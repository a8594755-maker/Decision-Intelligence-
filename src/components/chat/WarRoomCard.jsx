  ...
  return (
    <div className={`flex items-start gap-2 rounded-md ${style.bg} px-2.5 py-1.5 text-xs`}>
      ...
      <div className="min-w-0 flex-1">
        <div className="font-medium text-[var(--text-secondary)]">{finding.title}</div>
        <div className="text-[11px] text-[var(--text-muted)] mt-0.5">{finding.detail}</div>
      </div>
      <span className="text-[10px] font-mono text-[var(--text-muted)] shrink-0 capitalize">{finding.agent}</span>
    </div>
  );
}
...
export default function WarRoomCard({ payload, onActionClick }) {
  ...
  return (
    <Card category="risk" className="border border-[var(--border-default)] bg-gradient-to-br from-[var(--surface-card)] to-[var(--surface-base)] dark:to-[var(--surface-subtle)]">
      {/* Header */}
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <div className="p-1.5 rounded-md bg-[var(--surface-subtle)]">
      ...
      <div className="flex flex-wrap gap-1.5 mb-3">
        {agents_activated.map(agent => (
          <span
            key={agent}
            className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full bg-[var(--surface-subtle)] text-[var(--text-secondary)] border border-[var(--border-default)]"
          >
        ...
        <div className="space-y-1.5 mb-3">
          ...
          {findings.length > visibleFindings.length && !showAllFindings && (
            <button
              ...
              className="text-[10px] text-[var(--brand-600)] hover:underline flex items-center gap-0.5"
            >
        ...
        <div className="border-t border-[var(--border-default)] pt-2.5 mt-1">
          ...
          <div className="space-y-1.5">
            {recommendations.map((rec, i) => (
              <div
                key={rec.id || i}
                className="flex items-center gap-2 rounded-md border border-[var(--border-default)] bg-[var(--surface-card)] px-2.5 py-1.5"
              >
                <span className="text-[10px] font-mono text-[var(--text-muted)] capitalize shrink-0">
                  {rec.agent?.replace(/_/g, ' ')}
                </span>
                <span className="text-xs text-[var(--text-secondary)] flex-1">{rec.label}</span>
                ...
                {onActionClick && rec.action_type && (
                  <button
                    ...
                    className="shrink-0 p-1 rounded hover:bg-[var(--accent-active)] text-[var(--brand-600)]"
                  >
