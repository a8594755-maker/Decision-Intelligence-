...
export default function AnalysisInsightCard({ payload, onDeepDive }) {
    ...
    return (
      <div className="space-y-1">
        <div className="flex items-center gap-1.5">
          ...
          <h4 className="text-[11px] font-semibold text-[var(--text-secondary)] uppercase tracking-wide">Executive Summary</h4>
        </div>
    ...
    return (
      <div className="space-y-1.5">
        <div className="flex items-center gap-1.5">
          ...
          <h4 className="text-[11px] font-semibold text-[var(--text-secondary)] uppercase tracking-wide">{label}</h4>
        </div>
        <ul className="space-y-1 pl-5">
          {items.map((item, i) => (
            <li key={i} className="text-xs text-[var(--text-secondary)] leading-relaxed flex items-start gap-1.5">
    ...
    return (
      <div className="border-t border-[var(--border-default)] pt-2">
        <button
          ...
          className="flex items-center gap-1 text-[10px] font-medium text-[var(--text-muted)] hover:text-[var(--text-secondary)] transition-colors"
        >
        ...
        {expandedSources && (
          <ul className="mt-1 space-y-0.5 pl-4">
            {sources.map((src, i) => (
              <li key={i} className="text-[10px] text-[var(--text-muted)]">• {src}</li>
            ))}
          </ul>
        )}
      </div>
    ...
    return (
      <div className="border-t border-purple-100 dark:border-purple-800/50 pt-3 px-4 pb-3">
        <div className="flex items-center gap-1.5 mb-2">
          ...
          <h4 className="text-[11px] font-semibold text-[var(--text-secondary)] uppercase tracking-wide">Suggested Deep Dives</h4>
        </div>
  ...
  if (!hasStructuredSections && payload.rawMarkdown) {
    return (
      <div className="w-full rounded-xl border border-purple-200 dark:border-purple-800 border-l-[3px] border-l-[var(--cat-analysis)] bg-gradient-to-br from-purple-50/60 to-white dark:from-purple-950/20 dark:to-gray-900 shadow-sm overflow-hidden">
        <div className="px-4 pt-3 pb-2 flex items-center justify-between border-b border-purple-100 dark:border-purple-800/50">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold text-[var(--text-primary)]">AI Analysis Report</h3>
          </div>
          <span className="text-[10px] text-[var(--text-muted)] font-mono">{modelLabel}</span>
        </div>
        ...
        {renderDeepDives()}
        <div className="px-4 pb-2 text-[10px] text-[var(--text-muted)]">
  ...
  return (
    <div className="w-full rounded-xl border border-purple-200 dark:border-purple-800 border-l-[3px] border-l-[var(--cat-analysis)] bg-gradient-to-br from-purple-50/60 to-white dark:from-purple-950/20 dark:to-gray-900 shadow-sm overflow-hidden">
      ...
      <div className="px-4 pt-3 pb-2 flex items-center justify-between border-b border-purple-100 dark:border-purple-800/50">
        ...
        <span className="text-[10px] text-[var(--text-muted)] font-mono">{modelLabel} • {generatedAt || ''}</span>
      </div>
