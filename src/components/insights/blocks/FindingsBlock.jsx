      ...
      <ol className="space-y-2">
        {findings.map((f, i) => (
          <li key={i} className="flex gap-3 text-sm group">
            ...
            <span className="text-slate-600 dark:text-slate-400 leading-snug flex-1">{getText(f)}</span>
            {onAction && (
              <button
                onClick={() => onAction({ type: 'explore_insight', context: { title: `Finding: ${getText(f).slice(0, 80)}`, finding: getText(f) } })}
                className="shrink-0 opacity-0 group-hover:opacity-100 p-1 rounded text-slate-300 hover:text-blue-600 transition-all"
